...
class Features extends React.Component {
  ...
  async componentWillMount() {
    ...
    .then(() => {
      ...
      fetch('http://quill.technopathic.me/api/suggestFollows?token=' + this.state.token, {
        headers:{
  ...
  getFeature = () => {
    ...
    {
      fetch('http://quill.technopathic.me/api/getFollowTopics?page='+this.state.nextPage+'&token=' + this.state.token, {
        headers:{
          'Authorization': 'Bearer ' + this.state.token
        }
      })
  ...
  voteTopic(id, dir) {
    ...
    fetch('http://quill.technopathic.me/api/voteTopic/'+id+'?token=' + this.state.token, {
      method: 'POST',
      headers: {
  ...
  noFeeds()
  {
    ...
    if(this.state.topics.length === 0)
    {
      return(
        <Text style={noTopics}>
          Follow others to see their games.
        </Text>
      );
    }
  };
